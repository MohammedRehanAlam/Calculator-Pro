...
export const TimeConverter: React.FC = () => {
  ...
  const timeUnits = [
    { name: 'Milliseconds', symbol: 'ms', factor: 3600000 },
    { name: 'Seconds', symbol: 's', factor: 3600 },
    { name: 'Minutes', symbol: 'min', factor: 60 },
    { name: 'Hours', symbol: 'h', factor: 1 },
    { name: 'Days', symbol: 'days', factor: 0.0416667 },
    { name: 'Weeks', symbol: 'weeks', factor: 0.00595238 },
    { name: 'Months', symbol: 'months', factor: 0.00138889 },
    { name: 'Years', symbol: 'years', factor: 0.000114155 },
  ];

  const convertTime = (value: number, fromUnitName: string, toUnitName: string) => {
    const fromFactor = timeUnits.find(unit => unit.name === fromUnitName)?.factor || 1;
    const toFactor = timeUnits.find(unit => unit.name === toUnitName)?.factor || 1;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.screenHeader}>
        <Text style={styles.screenTitle}>Time Converter</Text>
      ...
      <ScrollView style={styles.converterContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.inputSection}>
          <Text style={styles.inputLabel}>Enter Time Value</Text>
          <TextInput
