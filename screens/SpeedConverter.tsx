...
export const SpeedConverter: React.FC = () => {
  ...
  const speedUnits = [
    ...
    { name: 'Kilometers per Hour', symbol: 'km/h', factor: 3.6 },
    { name: 'Miles per Hour', symbol: 'mph', factor: 2.237 },
    { name: 'Feet per Second', symbol: 'ft/s', factor: 3.281 },
    { name: 'Knots', symbol: 'kn', factor: 1.944 },
    { name: 'Mach', symbol: 'Ma', factor: 0.002938 },
  ];

  const convertSpeed = (value: number, fromUnitName: string, toUnitName: string) => {
    const fromFactor = speedUnits.find(unit => unit.name === fromUnitName)?.factor || 1;
    const toFactor = speedUnits.find(unit => unit.name === toUnitName)?.factor || 1;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.screenHeader}>
        <Text style={styles.screenTitle}>Speed Converter</Text>
      ...
      <ScrollView style={styles.converterContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.inputSection}>
          <Text style={styles.inputLabel}>Enter Speed Value</Text>
          <TextInput
        ...
        <View style={[styles.resultSection, { marginBottom: 16 }]}>
          ...
          <Text style={styles.resultText}>{getResult()} {speedUnits.find(u => u.name === toUnit)?.symbol}</Text>
        </View>
      </ScrollView>
    </View>
  );
